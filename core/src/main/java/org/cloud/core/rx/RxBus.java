...
public class RxBus {
    ...
    private <T> Flowable<T> toObservable(final int code, final Class<T> eventType) {
        return bus.toFlowable(BackpressureStrategy.BUFFER)
                ...
                .filter((Predicate<Message>) o -> o.getCode() == code && eventType.isInstance(o.getObject()))
                .map(Message::getObject).cast(eventType);
    }
    ...
    private void addSubscriber(final SubscriberMethod subscriberMethod) {
        ...
        Disposable subscription = postToObservable(flowable, subscriberMethod)
                .subscribe((Consumer<Object>) o -> callEvent(subscriberMethod, o));
